...
class opSimilarityMapper(Mapper):
    def __init__(self, infile=sys.stdin, separator='\t'):
        super(opSimilarityMapper, self).__init__(infile, separator)
        self.centroidStats = []
    ...
    def map(self):
        for record in self:
            ...
            for cs in self.centroidStats:     
                ...
                elif (bool((int(cs[0][-1]) + int(partition[0][-1])) % 2) != bool(int(cs[0]) > int(partition[0]))) and self.dist(pi, [int(i) for i in cs[0]]) < (cs[1] + self.t):
                    self.emit(cs[0], strpi + "," + "outer" + ","  + UserID)                     

if __name__ == "__main__":
    mapper = opSimilarityMapper()
    mapper.map()
